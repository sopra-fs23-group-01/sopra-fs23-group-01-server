...
public void userEditProfile(User user) {
  ...
  if(user.getBirthday()!=null){
    ...
    LocalDate newBirthday = LocalDate.of(localDate.getYear(), localDate.getMonth(), localDate.getDayOfMonth());
    
  
    LocalDateTime localDateTime = newBirthday.atStartOfDay();
    ZonedDateTime zdt1 = localDateTime.atZone(zoneId);
    Instant instant1 = zdt1.toInstant();
    Date date = Date.from(instant1);

  userByUserid.setBirthday(date);
  throw new ResponseStatusException(HttpStatus.OK, String.format((user.getBirthday()).toString(), "username", "is"));
  }
