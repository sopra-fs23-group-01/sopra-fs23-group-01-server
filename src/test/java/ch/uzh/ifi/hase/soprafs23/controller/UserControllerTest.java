 ...
 public class UserControllerTest {
     ...
     public void createUser_validInput() throws Exception {
         ...
         //whenever userservice.creatuser is used return user
     ...
     //create user with invalid input, to test whether the function createuser can be runned.
     @Test
     public void createUser_invalidInput() throws Exception {
         ...
         // when/then -> do the request + validate the result
         //Simulate the user request body
         MockHttpServletRequestBuilder postRequest = post("/users")
                 .contentType(MediaType.APPLICATION_JSON)
                 .content(asJsonString(userPostDTO));
     ...
     public void getUser_validId() throws Exception {
         ...
         // when/then -> do the request + validate the result
         when(userService.userProfileById(user.getId()))
                 .thenReturn(user);
     ...
     public void getUser_invalidId() throws Exception {
         ...
         given(userService.userProfileById((long) 1000))
     ...
     @Test
     public void testUserGetReady() throws Exception {
         // Prepare test data
         Long roomId = 1L;
         UserPutDTO userPutDTO = new UserPutDTO();
         // ... set properties of userPutDTO

         // Mock the service method
         User userInput = new User();
         doNothing().when(userService).userSetReady(Mockito.any());
         //when(DTOMapper.INSTANCE.convertUserPutDTOtoEntity(userPutDTO)).thenReturn(userInput);

         // Perform the PUT request
         mockMvc.perform(MockMvcRequestBuilders.put("/users/room/{roomId}", roomId)
                         .contentType(MediaType.APPLICATION_JSON)
                         .content(asJsonString(userPutDTO)))
     ...
     public void testUserLeaveRoom() throws Exception {
         ...
         doNothing().when(userService).userLeaveRoom(Mockito.any());
         doNothing().when(roomService).deletePlayer(Mockito.any(), Mockito.any());


         // Perform the PUT request
         mockMvc.perform(MockMvcRequestBuilders.put("/users/room/out/{roomId}", roomId)
                         .contentType(MediaType.APPLICATION_JSON)
                         .content(asJsonString(userPutDTO)))
                 .andExpect(MockMvcResultMatchers.status().isOk());
